...
static void set_start_screen(void) {
    ...
    gl_draw_string(180, 40, "BATTLESHIP", GL_WHITE);
...
static void set_end_screen(void) {                                           
    ...
    int clicks_x = centered_text_x(27);                                      
    gl_draw_string(clicks_x, 350, "ALL SHIPS SUNK IN    CLICKS", GL_WHITE);  
                                                                             
    char stat[3];                                                            
    stat_to_string(stat, clicks);                                       
    gl_draw_string(clicks_x + (18 * gl_get_char_width()), 350, stat, sunken); 
...
/* on_board returns true if the given (x, y) coord is located on the board portion of the screen
 */
static bool on_board(int x, int y) {
    return board_x <= x && x <= (board_x + board_width) && 
           board_y <= y && y <= (board_y + board_height);
}
...
static void read_mouse(void) {
    ...
    if(evt.left && evt.action == MOUSE_BUTTON_PRESS && on_board(mouse_x, mouse_y)) {
        if(screen == PLAY_SCREEN) {
            // calculate which square it is in
            int square_x = (mouse_x - board_x) / square_width;
            int square_y = (mouse_y - board_y) / square_height;
            if (display_hit(square_x, square_y))  // update only if not already hit
                store_cursor_colors(mouse_x, mouse_y);
        } else {
...
/* set_start_screen sets up the starting screen of the battleship game which includes an empty
...
static void set_play_screen(void) {
    ...
    gl_draw_string(stats_x + (gl_get_char_width() * 4), stats_y, "clicks: 00", GL_WHITE);
...
static void update_stats(void) {
    int click_stat_x = stats_x + (gl_get_char_width() * 12);

    // erase current stats
    gl_draw_rect(stats_x, stats_y, gl_get_char_width() * 2, gl_get_char_height(), background);
    gl_draw_rect(click_stat_x, stats_y, gl_get_char_width() * 2, gl_get_char_height(), background);
    ...
    stat_to_string(stat, clicks);
    gl_draw_string(click_stat_x, stats_y, stat, GL_WHITE);
}
...
/* play runs the battleship game in a forever loop of start screen, game, end screen
 */
